...
void UTargetingManagerComponent::BeginPlay()
{
	...
	const float stepAngle = (halfAngleDeg * 2.0f) / numSteps;

	// 각도 리스트를 '중심 우선'으로 생성
	// 순서: 0, +Step, -Step, +2Step, -2Step ...
	// 이 순서대로 트레이스를 하면 결과가 자동으로 중심 우선 정렬됨.
	AnglesOrdered.Reserve(numSteps + 1);

	// 0도 (중심) 먼저 추가
	AnglesOrdered.Add(0.0f);

	// 중심에서 벌어지며 추가
	for (int32 i = 1; i <= numSteps / 2; ++i)
	{
		float currentStep = stepAngle * i;
		if (currentStep <= halfAngleDeg)
		{
			AnglesOrdered.Add(currentStep);  // 양수 각도
			AnglesOrdered.Add(-currentStep); // 음수 각도
		}
	}
}
...
AActor* UTargetingManagerComponent::FindNearEnemyToTarget(float RadiusOverride)
{
	...
	FVector endLocation = startLocation + traceDirection * GetFindDistance(!bForcePreventActionLaunchStat);
	...
	//공격 중일때 (막 끝났을때)
	if (!OwnerCharacter.Get()->ActionTrackingComponent->GetIsActionReady(ECharacterActionType::E_Attack)
		|| !OwnerCharacter.Get()->ActionTrackingComponent->GetIsActionReady(ECharacterActionType::E_JumpAttack)
		|| !OwnerCharacter.Get()->ActionTrackingComponent->GetIsActionReady(ECharacterActionType::E_DashAttack))
	{
		...
		endLocation = startLocation + traceDirection * GetFindDistance(!bForcePreventActionLaunchStat);
...
void UTargetingManagerComponent::UpdateCameraRotation()
{
	...
	if (OwnerCharacter.Get()->ActionTrackingComponent->GetIsActionInProgress(ECharacterActionType::E_Skill)) return;
	...
	bool bIgnoreSelf, FLinearColor TraceColor, FLinearColor TraceHitColor, float DrawTime)
{
	...
	// 1. 기준 데이터 설정 (지역변수 camelCase)
	const FVector forwardVec = (End - Start).GetSafeNormal();
	const float maxDistance = (End - Start).Size();
	const FRotator baseRot = forwardVec.Rotation();

	// 4. 트레이스 실행 (Single Trace)
	TSet<AActor*> processedActors; // 중복 방지용 (이미 찾은 적은 다른 각도에서 무시)

	for (float angle : AnglesOrdered)
	{
		FRotator rotatedYaw = baseRot + FRotator(0.0f, angle, 0.0f);
		FVector dir = rotatedYaw.Vector();
		FVector traceEnd = Start + dir * maxDistance;

		FHitResult singleHit;

		// [최적화] Multi -> Single 변경
		// 해당 각도에서 가장 먼저 닿는 물체 하나만 반환
		bool bHit = UKismetSystemLibrary::SphereTraceSingleByProfile(
			...
			DrawDebugType, singleHit, bIgnoreSelf, TraceColor, TraceHitColor, DrawTime
		);
		...
		if (bHit)
		{
			...
			{
				// 결과 추가 (이미 각도 순서대로 루프를 돌기 때문에 정렬 불필요)
...
float UTargetingManagerComponent::GetFindDistance(bool bContainActionLaunchStat)
{
	if(bContainActionLaunchStat && OwnerCharacter.IsValid() && OwnerCharacter->ActorHasTag("Player"))
	{
		return FMath::Max(MaxFindDistance, Cast<AMainCharacterBase>(OwnerCharacter.Get())->GetStatTotalValue(ECharacterStatType::E_ActionLaunchDistance));
	}
	return MaxFindDistance;
}
...
bool UTargetingManagerComponent::GetIsEnemyInBoundary(FVector EnemyLocation)
{
	...
	// 거리 조건 추가 (Player이면, ActionLaunchDistance 사용, Enemy이면 MaxFindDistance 사용)
	float targetDistance = GetFindDistance(!bForcePreventActionLaunchStat);
	bool isWithinDistance = FVector::Dist(playerLocation, EnemyLocation) <= targetDistance;
