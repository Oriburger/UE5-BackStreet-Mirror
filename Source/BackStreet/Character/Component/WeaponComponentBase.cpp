...
void UWeaponComponentBase::InitWeaponAsset()
{
	...
	if (WeaponAssetInfo.MeleeWeaponAssetInfo.HitImpactSound.IsValid())
	{
		HitEffectSound = WeaponAssetInfo.MeleeWeaponAssetInfo.HitImpactSound.Get();
	}
	if (WeaponAssetInfo.MeleeWeaponAssetInfo.HitImpactSoundLarge.IsValid())
	{
		HitEffectSoundLarge = WeaponAssetInfo.MeleeWeaponAssetInfo.HitImpactSoundLarge.Get();
	}
}
...
float UWeaponComponentBase::CalculateTotalDamage(FCharacterGameplayInfo TargetState, bool& bIsFatalAttack)
{
	...
	bool bIsSkillAttacking = OwnerCharacterRef.Get()->ActionTrackingComponent->GetIsActionInProgress(ECharacterActionType::E_Skill);
	...
	bool bIsJumpAttacking = !OwnerCharacterRef.Get()->ActionTrackingComponent->GetIsActionReady(ECharacterActionType::E_JumpAttack);

	float gearCount = Cast<AMainCharacterBase>(OwnerCharacterRef.Get())->GetCurrentGearCount();
	...
	// E_RichProcessing 적용 로직
	// 스탯 수치는 '기어 1개당 증가할 데미지 비율'을 의미함 (예: 0.005f)
	float richStat = ownerState.GetTotalValue(ECharacterStatType::E_RichProcessing);

	if (richStat > 0.0f)
	{
		// 1. 기어에 따른 보너스 계산 (richStat이 0.01일 때, 200개 모으면 2.0(200%) 달성)
		float richBonus = gearCount * richStat;

		// 2. 최대 증가폭을 200%(2.0f)로 제한하여 말도 안 되게 강해지는 것 방지
		float finalBonus = FMath::Min(richBonus, 2.0f);

		// 3. 최종 공격력에 곱연산 적용
		attackPower *= (1.0f + finalBonus);

		UE_LOG(LogWeapon, Log, TEXT("UWeaponComponentBase::CalculateTotalDamage() - Rich Processing applied: GearCount=%f, RichStat=%f, FinalBonus=%f, NewAttackPower=%f"),
			gearCount, richStat, finalBonus, attackPower);
	}
...
bool UWeaponComponentBase::ApplyWeaponDebuff(ACharacterBase* TargetCharacter, ECharacterDebuffType DebuffType, ECharacterStatType DebuffStatType)
{
	...
	/* 디버프 확률 적용 로직 제거(26.02.10)
	if (targetValue > 0.0f && probabilityValue * 100.0f > randomSeed)
	{
		result = TargetCharacter->TryAddNewDebuff(FDebuffInfoStruct(DebuffType, targetValue, WeaponStat.DebuffInfo.StackAmount, true), OwnerCharacterRef.Get());
	}
	*/
	...
	if (OwnerCharacterRef.Get()->ActorHasTag("Player"))
	{
		if (targetValue > 0.0f)
		{
			TargetCharacter->ApplyDebuffStack(FDebuffInfoStruct(DebuffType, WeaponStat.DebuffInfo.StackAmount), OwnerCharacterRef.Get());
			result = true;
		}
	}
	else if(OwnerCharacterRef.Get()->ActorHasTag("Enemy") && DebuffType == WeaponStat.DebuffInfo.Type)
	{
		TargetCharacter->ApplyDebuffStack(FDebuffInfoStruct(DebuffType, WeaponStat.DebuffInfo.StackAmount), OwnerCharacterRef.Get());
		result = true;
	}
