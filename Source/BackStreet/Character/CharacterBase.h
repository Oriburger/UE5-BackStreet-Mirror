...
//====== Delegate ========================================================
public:
	FDelegateOnActionTrigger* GetActionTriggerDelegate(ECharacterActionType Type, bool bIsEndDelegate = false);

	bool TryBroadcastActionTriggerDelegate(ECharacterActionType Type, bool bIsEndDelegate = false);
...
public:
	TMap<FName, TUniquePtr<FDelegateOnActionTrigger>> ActionTriggerDelegateMap;
...
public:
	...
	//디버프 데미지를 입힘 (일회성)
	UFUNCTION(BlueprintCallable)
		virtual float TakeDebuffDamage(ECharacterDebuffType DebuffType, float DamageAmount, AActor* Causer);

	UFUNCTION(BlueprintCallable)
		virtual bool ApplyDebuffStack(FDebuffInfoStruct DebuffInfo, AActor* Causer);
	...
	UFUNCTION(BlueprintCallable)
		void SetLocationWithInterp(FVector StartLocation, FVector TargetLocation, TArray<TEnumAsByte<EObjectTypeQuery>> ObjectTypeList, float InterpSpeed = 1.0f, const bool bAutoReset = false
				, const bool bUseSafeEndLocation = true, const bool bUseSafeInterpBlock = false, const bool bIgnoreZAxis = false, const bool bDrawDebugInfo = false);
...
protected:
	...
	UFUNCTION()
		void CheckSafeLocation(const FVector TargetValue);
...
public:
	...
	UFUNCTION(BlueprintCallable, BlueprintPure)
		FCharacterDefaultStat& GetCharacterDefaultStatRef() { return DefaultStat; }
	...
	UFUNCTION(BlueprintCallable, BlueprintPure)
		bool GetIsLocationInterping() { return GetWorldTimerManager().IsTimerActive(LocationInterpHandle); }
