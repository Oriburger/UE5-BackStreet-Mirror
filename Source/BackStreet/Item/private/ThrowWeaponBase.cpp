...
FProjectileAssetInfoStruct AThrowWeaponBase::GetProjectileAssetInfo(int32 TargetProjectileID)
{
	//캐시에 기록이 되어있다면?
...
void AThrowWeaponBase::ClearAllTimerHandle()
{
	Super::ClearAllTimerHandle();
	GetWorldTimerManager().ClearTimer(AutoReloadTimerHandle);
} 

void AThrowWeaponBase::UpdateWeaponStat(FWeaponStatStruct NewStat)
{
	//WeaponStat 업데이트
}

void AThrowWeaponBase::InitWeaponAsset()
{
...
void AThrowWeaponBase::SpawnShootNiagaraEffect()
{
...
AProjectileBase* AThrowWeaponBase::CreateProjectile()
{
...
bool AThrowWeaponBase::GetCanReload()
{	
...
void AThrowWeaponBase::AddMagazine(int32 Count)
{
	if (WeaponStat.RangedWeaponStat.bIsInfiniteAmmo || WeaponState.RangedWeaponState.ExtraAmmoCount >= MAX_AMMO_LIMIT_CNT) return;
