...
void UStageManagerComponent::BeginPlay()
{
	...
	{
		FDelegateOnActionTrigger* deathDelegate = playerCharacter->GetActionTriggerDelegate(ECharacterActionType::E_Die);
		if (deathDelegate)
		{
			deathDelegate->AddUObject(this, &UStageManagerComponent::SetGameIsOver);
		}
	}
...
TArray<AActor*> UStageManagerComponent::TryUpdateSpawnPointProperty()
{
	...
	CurrentStageInfo.ClearSpawnLocationInfo();
	CurrentStageInfo.bIsDividedArea = true;
	...
	for (AActor*& spawnPoint : spawnPointList)
	{
		...
		if (spawnPoint->Tags[1] == FName("Enemy"))
		{
			...
			//tag에 index가 전부 존재하면 bIsDividedArea를 true로 설정
			if (spawnPoint->Tags.IsValidIndex(2) && CurrentStageInfo.bIsDividedArea)
			{
				CurrentStageInfo.bIsDividedArea &= spawnPoint->Tags.IsValidIndex(2); //Tag[2]가 존재하는지 체크, bIsDividedArea에 AND 연산
				
				//EnemySpawnLocationIdxList에 스폰 포인트의 Tag[2] 값을 int로 변환하여 추가
				CurrentStageInfo.EnemySpawnLocationIdxList.Add(FCString::Atoi(*spawnPoint->Tags[2].ToString()));
				CurrentStageInfo.MaxAreaIdx = FMath::Max(CurrentStageInfo.MaxAreaIdx, CurrentStageInfo.EnemySpawnLocationIdxList.Last());
			}
			else
			{
				CurrentStageInfo.bIsDividedArea = false;
			}
		}
	...
	//Area 관련 변수 초기화
	CurrentStageInfo.bIsDividedArea &= !CurrentStageInfo.EnemySpawnLocationList.IsEmpty(); //스폰 포인트가 하나도 없으면 구역 분할 불가
	if(!CurrentStageInfo.bIsDividedArea)
	{
		CurrentStageInfo.EnemySpawnLocationIdxList.Empty();
		CurrentStageInfo.MaxAreaIdx = -1;
	}

	return spawnPointList;
}

void UStageManagerComponent::SpawnEnemy(int32 AreaIdx)
{
	//Basic condition (stage type check and data count check
	if (!CurrentStageInfo.GetIsCombatStage()) return;
	if (!CurrentStageInfo.bIsDividedArea && AreaIdx >= 0)
	{
		UE_LOG(LogStage, Error, TEXT("UStageManagerComponent::SpawnEnemy : Area division is not supported in this stage"));
		return;
	}
	if(CurrentStageInfo.bIsDividedArea && CurrentStageInfo.EnemySpawnLocationIdxList.Num() != CurrentStageInfo.EnemySpawnLocationList.Num())
	{
		UE_LOG(LogStage, Error, TEXT("UStageManagerComponent::SpawnEnemy : EnemySpawnLocationIdxList count is not equal to EnemySpawnLocationList count, idx list : %d, location list : %d")
			, CurrentStageInfo.EnemySpawnLocationIdxList.Num(), CurrentStageInfo.EnemySpawnLocationList.Num());
		return;
	}

	//--------------Spawn boss character to world -----------------------------
	//--------------boss stage doesn't support area division yet --------------
	if (CurrentStageInfo.StageType == EStageCategoryInfo::E_Boss)
	{
		...
		UE_LOG(LogStage, Warning, TEXT("UStageManagerComponent::SpawnEnemy : Boss Stage - Boss will be spawned"));
		...
		AEnemyCharacterBase* boss = GetWorld()->SpawnActor<AEnemyCharacterBase>(CurrentChapterInfo.BossCharacterClass
			, CurrentStageInfo.BossSpawnLocation, CurrentStageInfo.BossSpawnRotation, spawnParams);
		...
		{
			UE_LOG(LogStage, Error, TEXT("UStageManagerComponent::SpawnEnemy : Boss Stage - Boss spawn failed, class name : %s, spawn location : %s"), *CurrentChapterInfo.BossCharacterClass->GetName(), *CurrentStageInfo.BossSpawnLocation.ToCompactString());
		}
		return;
	}

	//-------------Check enemy composition data-------------------------------
	...
	for (int32 idx = 0; idx < CurrentStageInfo.EnemySpawnLocationList.Num(); idx++)
	{
		...
		if(CurrentStageInfo.bIsDividedArea && CurrentStageInfo.EnemySpawnLocationIdxList[idx] != AreaIdx && AreaIdx >= 0)
		{
			UE_LOG(LogStage, Log, TEXT("UStageManagerComponent::SpawnEnemy : Skip spawning at area %d"), CurrentStageInfo.EnemySpawnLocationIdxList[idx]);
			continue;
		}
...
void UStageManagerComponent::SpawnPortal(int32 GateCount)
{
	...
	for (int32 idx = 0; idx < GateCount; idx++)
	{
		...
		if (IsValid(newGate))
		{
			...
			newGate->OnEnterRequestReceived.BindUFunction(this, FName("OnGateEnterBegin"));
...
void UStageManagerComponent::OnGateEnterBegin(int32 GateIdx)
{
	if (!ChapterManagerRef.IsValid()) return;
	ChapterManagerRef.Get()->MoveStage(GateIdx);
}

void UStageManagerComponent::StartStage()
{
	//Load End Broadcast (Safe with latent delay)
	UE_LOG(LogStage, Log, TEXT("UStageManagerComponent::StartStage : OnStageLoadDone Broadcast"));
	...
	SpawnEnemy(CurrentStageInfo.bIsDividedArea ? 0 : -1);
	CurrentStageInfo.CurrentAreaIdx = CurrentStageInfo.bIsDividedArea ? 0 : -1;

	//sub reward
	if (CurrentStageInfo.SubRewardInfo.ItemID != 0
		&& CurrentStageInfo.SubRewardInfoCandidateList.Num() > 0)
	{
		UE_LOG(LogStage, Warning, TEXT("UStageManagerComponent::StartStage : GrantStageRewards (sub reward)"));
		GrantStageRewards(true); //sub reward
	}
...
bool UStageManagerComponent::TryStartNewArea()
{
	if(CurrentStageInfo.bIsClear || CurrentStageInfo.bIsGameOver)
	{
		UE_LOG(LogStage, Error, TEXT("UStageManagerComponent::StartArea : Stage is already finished"));
		return false;
	}
	if (!CurrentStageInfo.bIsDividedArea
		|| CurrentStageInfo.CurrentAreaIdx > CurrentStageInfo.MaxAreaIdx)
	{
		UE_LOG(LogStage, Error, TEXT("UStageManagerComponent::StartArea : Area division is not supported in this stage or AreaIdx is out of range"));
		return false;
	}
	SpawnEnemy(CurrentStageInfo.CurrentAreaIdx);
	OnAreaBegin.Broadcast(CurrentStageInfo.CurrentAreaIdx);
	return true;
}

void UStageManagerComponent::FinishCurrentArea()
{
	if (!CurrentStageInfo.bIsDividedArea)
	{
		UE_LOG(LogStage, Error, TEXT("UStageManagerComponent::FinishArea : Area division is not supported in this stage"));
		return;
	}
	//Stage Reward
	GrantStageRewards();
	OnAreaDone.Broadcast(CurrentStageInfo.CurrentAreaIdx);
}

void UStageManagerComponent::GrantStageRewards(bool bIsSubReward)
{
	AMainCharacterBase* playerCharacter = Cast<AMainCharacterBase>(UGameplayStatics::GetPlayerCharacter(GetWorld(), 0));

	if (!IsValid(playerCharacter)) return;

	FItemInfoDataStruct rewardItem = bIsSubReward ? GetCurrentStageInfo().SubRewardInfo : GetCurrentStageInfo().MainRewardInfo;
	
	UE_LOG(LogStage, Log, TEXT("UStageManagerComponent::GrantStageRewards - ItemID : %d, Amount : %d, bIsActorItem : %d"), 
		rewardItem.ItemID, rewardItem.ItemAmount, rewardItem.bIsActorItem);

	if (rewardItem.ItemID == 2)
	{
		rewardItem.ItemAmount = 30;
	}

	if (rewardItem.bIsActorItem)
	{
		SpawnRewardActor(rewardItem, bIsSubReward);
	}
	else
	{
		playerCharacter->ItemInventory->AddItem(rewardItem.ItemID, rewardItem.ItemAmount);
	}
}

AActor* UStageManagerComponent::SpawnRewardActor(FItemInfoDataStruct ItemInfo, bool bIsSubReward)
{
	UE_LOG(LogStage, Log, TEXT("UStageManagerComponent::SpawnRewardActor - ItemID : %d, bIsActorItem : %d, bIsSubReward : %d"), 
		ItemInfo.ItemID, ItemInfo.bIsActorItem, bIsSubReward);
	...
	FVector spawnLocation = (bIsSubReward ? CurrentStageInfo.PlayerStartLocation : CurrentStageInfo.RewardSpawnLocation) + FVector(0, 0, 100);
...
void UStageManagerComponent::UpdateEnemyCountAndCheckClear()
{
	...
	{
		if (!CurrentStageInfo.bIsDividedArea || CurrentStageInfo.CurrentAreaIdx == CurrentStageInfo.MaxAreaIdx)
		{
			UE_LOG(LogStage, Log, TEXT("UStageManagerComponent::UpdateEnemyCountAndCheckClear - Stage Clear!"));
			//Stage Reward if all areas are cleared	
			if(CurrentStageInfo.bIsDividedArea)
			{
				GrantStageRewards();
			}
		...
		else if (CurrentStageInfo.bIsDividedArea)
		{
			FinishCurrentArea();
			CurrentStageInfo.CurrentAreaIdx += 1;
			UE_LOG(LogStage, Log, TEXT("UStageManagerComponent::UpdateEnemyCountAndCheckClear - AreaCleared! Move to Area %d"), CurrentStageInfo.CurrentAreaIdx);
		}
	}
}
