...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FDelegateAreaBegin, int32, AreaIdx);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FDelegateAreaDone, int32, AreaIdx);
...
public:
	...
	UPROPERTY(BlueprintAssignable, VisibleAnywhere, BlueprintCallable)
		FDelegateAreaBegin OnAreaBegin;

	UPROPERTY(BlueprintAssignable, VisibleAnywhere, BlueprintCallable)
		FDelegateAreaDone OnAreaDone;
...
protected:
	...
	//Spawn enemy character, area idx : -1 means all area
	UFUNCTION()
		void SpawnEnemy(int32 AreaIdx = -1);
	...
	UFUNCTION()
		void OnGateEnterBegin(int32 GateIdx);
...
public:
	...
	UFUNCTION(BlueprintCallable)
		bool TryStartNewArea();

	UFUNCTION()
		void FinishCurrentArea();
...
private:
	//Grant stage reward
	UFUNCTION()
		void GrantStageRewards(bool bIsSubReward = false);

	UFUNCTION()
		AActor* SpawnRewardActor(FItemInfoDataStruct ItemInfo, bool bIsSubReward = false);
