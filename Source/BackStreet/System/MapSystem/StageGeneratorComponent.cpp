...
TArray<FStageInfo> UStageGeneratorComponent::Generate()
{
	...
	for (int32 stageIdx = 0; stageIdx < stageCount; stageIdx++)
	{
		...
		if (stageInfo.GetIsCombatStage(false))
		{
			...
			stageInfo.MainRewardInfoCandidateList = GetRewardListFromCandidates(stageInfo.StageType, {});
			stageInfo.SubRewardInfoCandidateList = GetRewardListFromCandidates(stageInfo.StageType, {}, true);

			//print all reward candidate info  through uelog
			for(auto& itemInfo : stageInfo.MainRewardInfoCandidateList)
			{
				UE_LOG(LogStage, Warning, TEXT("UStageGeneratorComponent::Generate() - Main Reward Item : %s"), *itemInfo.ItemName.ToString());
			}
			for (auto& itemInfo : stageInfo.SubRewardInfoCandidateList)
			{
				UE_LOG(LogStage, Warning, TEXT("UStageGeneratorComponent::Generate() - Sub Reward Item : %s"), *itemInfo.ItemName.ToString());
			}

			//고정 스킬 보상 스테이지를 설정한다.
			if (bIsFixedSkillRewardStage)
			{
				stageInfo.SubRewardInfoCandidateList.Insert(GetRewardItemInfo(CurrentChapterInfo.SkillItemID), 0);
			...
			UE_LOG(LogStage, Warning, TEXT("UStageGeneratorComponent::Generate() - Stage Type : %d, Reward Count : %d, FixedSkillRewardStage : %d, SkillRewardCount : %d")
				, (int32)stageInfo.StageType, stageInfo.MainRewardInfoCandidateList.Num(), (int32)bIsFixedSkillRewardStage, CurrentChapterInfo.CurrentSkillRewardCount);
		}
...
TArray<FItemInfoDataStruct> UStageGeneratorComponent::GetRewardListFromCandidates(EStageCategoryInfo StageType, TArray<int32> ExceptIDList, bool bIsSubStage)
{
	TMap<EStageCategoryInfo, FStageRewardCandidateInfoList>& targetRewardMap = bIsSubStage ?
		CurrentChapterInfo.StageSubRewardCandidateInfoMap : CurrentChapterInfo.StageRewardCandidateInfoMap;

	if (!targetRewardMap.Contains(StageType)) return {};

	const FStageRewardCandidateInfoList& rewardInfoList = *targetRewardMap.Find(StageType);
