...
void UAnimNotifyState_SlashMultiEnemy::NotifyTick(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, float FrameDeltaTime, const FAnimNotifyEventReference& EventReference)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("NotifyTick #1 Start : Checking mesh visibility : %d"), MeshComp->IsVisible());
	...
	if (SlashIdx >= resultList.Num() || CurrSlashCount >= MaxSlashCount)
	{
	...
	UE_LOG(LogTemp, Warning, TEXT("NotifyTick #2 Start : Checking mesh visibility : %d"), MeshComp->IsVisible());
	...
	if (ElapsedTime >= SlashInterval)
	{
		...
		if (IsValid(resultList[targetIdx]))
		{
			...
			targetLocation.Z += ZLocationOffset;

			bIsInterping = true;
			OwnerCharacterRef.Get()->SetLocationWithInterp(OwnerCharacterRef.Get()->GetActorLocation(), targetLocation, objectTypeList, InterpSpeed, false, false, false, false, false);
	...
	UE_LOG(LogTemp, Warning, TEXT("NotifyTick #3 Start : Checking mesh visibility : %d"), MeshComp->IsVisible());
}
